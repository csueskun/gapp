...
class POS{
    ...
    public static function comandaPosStack($pedido,$config,$re=false){
        ...
        foreach ($pedido->productos_pedido as $producto_pedido) {
            ...
            try {
                $tipoProducto = $producto_pedido->producto->tipo_producto;
                $impresora_dedicada = $tipoProducto && $tipoProducto->impresora 
                    ? $tipoProducto->impresora : $config->impresora_comanda;
            } catch (\Throwable $th) {
                $impresora_dedicada = $config->impresora_comanda;
            }
            $impresora_dedicada = trim($impresora_dedicada);
            $impresora_dedicada = explode(',', $impresora_dedicada);
            ...
            $stack[] = ["i"=>"doble","v"=>2];
            foreach ($impresora_dedicada as $impresora_name) {
                $stack[] = ["i"=>"producto_pedido","v"=>$texto, "impresora"=>$impresora_name];
            }
            $stack[] = ["i"=>"sencilla","v"=>1];
