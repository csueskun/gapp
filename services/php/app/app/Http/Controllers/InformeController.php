...
class InformeController extends Controller
{
    ...
    public function tortaTipoProducto($ano = null, $mes = null) {
        ...
        if($ano!=null){
            if($mes == null){
                $ano_mes = "and gapp_pedido.created_at >= '$ano-01-01 00:00:00' and gapp_pedido.created_at < '".($ano+1)."-01-01 00:00:00'";
            }
            else{
                $ano_mes = "and gapp_pedido.created_at >= '$ano-$mes-01 00:00:00' and gapp_pedido.created_at < '$ano-".($mes+1)."-01 00:00:00'";
            }
        }
    ...
    SELECT tp.descripcion,
        (
            select coalesce(sum(gapp_producto_pedido.total),0) from gapp_pedido
            join gapp_producto_pedido
            on gapp_pedido.id = gapp_producto_pedido.pedido_id
            join gapp_producto
            on gapp_producto_pedido.producto_id = gapp_producto.id
            join gapp_tipo_producto
            on gapp_producto.tipo_producto_id = gapp_tipo_producto.id
            where gapp_tipo_producto.id = tp.id $ano_mes
        ) as total
        FROM gapp_tipo_producto tp order by 2 desc
        ");
    }
    public function tortaTipoProductoRango($ano, $mes, $ano2, $mes2) {
        ...
        $ano_mes = "and gapp_pedido.created_at >= '$ano-$mes-01 00:00:00' and gapp_pedido.created_at < '$ano2-$mes2-01 00:00:00' ";
    ...
    public function anosConPedido(){
        $sql = DB::select("
            select distinct(YEAR(gapp_pedido.created_at)) as ano from gapp_pedido order by 1
        ");
        return $sql;
    }
    ...
    public function pedidosAno(){
        $sql = DB::select("
        SELECT YEAR(gapp_pedido.created_at) as ano, SUM(gapp_pedido.total) as suma
        FROM gapp_pedido
        GROUP BY YEAR(gapp_pedido.created_at) order by 1
        ");
        return $sql;
    }
    ...
    public function pedidosMes($ano){
        $sql = DB::select("
        SELECT MONTH(gapp_pedido.created_at) as mes, SUM(gapp_pedido.total) as suma
        FROM gapp_pedido
        where gapp_pedido.created_at >= '$ano-01-01 00:00:00' and gapp_pedido.created_at < '".($ano+1)."-01-01 00:00:00'
        GROUP BY MONTH(gapp_pedido.created_at) order by 1
        ");
        return $sql;
    }
    ...
    public function sqlMasVendido() {
        ...
        $sql = "SELECT tp.descripcion as tipo,p.descripcion, sum(d.cantidad) as cantidad, SUM(d.total) as total
            FROM gapp_detalle_documento d
            INNER JOIN gapp_documento pd ON(pd.id = d.documento_id)  
            INNER JOIN gapp_producto p ON(p.id = d.producto_id )
            INNER JOIN gapp_tipo_producto tp ON(p.tipo_producto_id = tp.id)
            $cond
            GROUP BY 1,2
    ...
    from gapp_documento d inner join gapp_detalle_documento dd on(d.id = dd.documento_id)
     inner join gapp_producto  p on(p.id = dd.producto_id)
     inner join gapp_tipo_producto tp on(tp.id = p.tipo_producto_id)
     inner join gapp_users u on(d.usuario_id = u.id)
     where tipodoc = 'FV'
    ...
    public function informeBanco() {
        ...
        sum(paga_credito) as credito, sum(paga_transferencia) as transferencia, 
        sum(paga_plataforma) as plataforma, sum(paga_puntos) as puntos, 
        sum(paga_efectivo + paga_debito + paga_credito + paga_transferencia + paga_plataforma + paga_puntos) as total
        from gapp_documento 
