...
class SaldosProductoController extends Controller
{
    ...
    function generarDetallado(){
        ...
        if($postData['tipo']=='ING'){
            ...
            inner join gapp_documento d on(dd.documento_id = d.id)
            inner join gapp_producto_pedido pp on(d.pedido_id = pp.pedido_id and dd.producto_id = pp.producto_id)
            INNER join gapp_producto_pedido_ingrediente pi on(pi.producto_pedido_id = pp.id )
            inner join gapp_ingrediente i on(pi.ingrediente_id = i.id )
            ...
            inner join gapp_documento d on(dd.documento_id = d.id)
            inner join gapp_producto_pedido pp on(d.pedido_id = pp.pedido_id and dd.producto_id = pp.producto_id)
            INNER join gapp_producto_pedido_adicional pa on(pa.producto_pedido_id = pp.id )
            inner join gapp_adicional a on(pa.adicional_id = a.id)
            inner join gapp_ingrediente i on(a.ingrediente_id = i.id )
            where i.id = ".$postData['id']."
            order by 1 ASC
            ";
        }
        ...
        FROM gapp_detalle_documento pdc
        JOIN gapp_documento as pd
...
//    }
    ...
    public function pos(){
        ...
        coalesce((SELECT SUM(cantidad) FROM gapp_documento d
          INNER JOIN gapp_detalle_documento dd
        ...
        FROM gapp_saldos_producto  s
        inner join gapp_producto p ON(s.producto_id = p.id)
        ...
        FROM gapp_saldos_producto  s
        inner join gapp_ingrediente i ON(s.ingrediente_id = i.id)
