...
class ConfigController extends Controller
{
    ...
    public function resetTurno() {
        try {
            ...
            if($now>$desde){
                $cantidad = DB::select("select count(id) as c from gapp_pedido where created_at > '$desde'");
                if($cantidad[0]->c > 0){
                }
                else{
    ...
    public function prepararImpresoraPos($printer, $width){
        $impresora = [
            'impresora' => $printer,
            'num_impresora' => $width,
        ];
        ...
        $printService->print($impresora, $rows, $options);
    }
