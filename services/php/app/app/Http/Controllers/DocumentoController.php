...
class DocumentoController extends Controller
{
    ...
    public function cuadre() {
        ...
        if($caja_id == '0'){
            ...
            $caja_condicion = "and gapp_documento.caja_id = $caja_id";
            $caja_condicion_d = "and d.caja_id = $caja_id";
            $caja_condicion_p = "and p.caja_id = $caja_id";
            $caja_condicion_f = "and caja_id = $caja_id";
            $caja_condicion_fp = "and gapp_pedido.caja_id = $caja_id";
        }

        $cuadre = DB::select("
            Select 'I' as ie, 'BI' as tipo, COALESCE(sum(total),0) as total 
            from gapp_documento where gapp_documento.tipodoc = 'BI' 
            and gapp_documento.created_at >= $fecha_inicio 
            and gapp_documento.created_at <= $fecha_fin 
            $caja_condicion
            
            UNION ALL
            Select 'E' as ie, 'PN' as tipo, COALESCE(sum(total),0) as total 
            from gapp_documento where gapp_documento.tipodoc = 'PN' 
            and gapp_documento.created_at >= $fecha_inicio 
            and gapp_documento.created_at <= $fecha_fin 
            $caja_condicion
            ...
            from gapp_documento
            where gapp_documento.created_at >= $fecha_inicio 
            and gapp_documento.created_at <= $fecha_fin 
            $caja_condicion
            ...
            from gapp_pedido
            where gapp_pedido.created_at >= $fecha_inicio 
            and gapp_pedido.created_at <= $fecha_fin and gapp_pedido.estado = 2 
            $caja_condicion_fp
            
            UNION ALL
            Select 'I' as ie, 'FV' as tipo, COALESCE(sum(total),0) as total 
            from gapp_documento where gapp_documento.tipodoc = 'FV' 
            and gapp_documento.created_at >= $fecha_inicio 
            and gapp_documento.created_at <= $fecha_fin 
            $caja_condicion
            ...
            from gapp_documento where gapp_documento.tipodoc = 'FC' 
            and gapp_documento.created_at >= $fecha_inicio 
            and gapp_documento.created_at <= $fecha_fin 
            $caja_condicion
            ...
            from gapp_documento where gapp_documento.tipodoc = 'CI' 
            and gapp_documento.created_at >= $fecha_inicio 
            and gapp_documento.created_at <= $fecha_fin 
            $caja_condicion
            ...
            from gapp_documento where gapp_documento.tipodoc = 'CE' 
            and gapp_documento.created_at >= $fecha_inicio 
            and gapp_documento.created_at <= $fecha_fin 
            $caja_condicion
            ...
            from gapp_documento where gapp_documento.tipodoc = 'RC' 
            and gapp_documento.created_at >= $fecha_inicio 
            and gapp_documento.created_at <= $fecha_fin 
            $caja_condicion
            ...
            from gapp_documento where gapp_documento.tipodoc = 'RT' 
            and gapp_documento.created_at >= $fecha_inicio 
            and gapp_documento.created_at <= $fecha_fin 
            $caja_condicion
            ...
            and gapp_documento.created_at <= $fecha_fin 
            $caja_condicion 
            and ( gapp_documento.tipodoc = 'BI' or gapp_documento.tipodoc = 'PN'
            or gapp_documento.tipodoc = 'FV'    or gapp_documento.tipodoc = 'CI' 
            or gapp_documento.tipodoc = 'RC'    or gapp_documento.tipodoc = 'FC'
            or gapp_documento.tipodoc = 'CE'    or gapp_documento.tipodoc = 'RT')
            ");
        
        $fv = DB::select("
            select concat(tp.descripcion,' ',JSON_EXTRACT(pp.obs, '$.tamano')) as descripcion, 
            sum(pp.cant) as cantidad, sum(pp.total) as total, numdoc, d.id from gapp_documento d

            join gapp_pedido as p
            on p.id = d.pedido_id

            join gapp_producto_pedido as pp
            on p.id = pp.pedido_id

            join gapp_producto as pr
            on pr.id = pp.producto_id

            join gapp_tipo_producto as tp
            ...
            from gapp_documento where fecha_anulado is null and tipodoc = 'FV' and (pedido_id = 0 or pedido_id is NULL)
        ...
        $anulados = DB::select("
            select tipodoc, codprefijo, numdoc from gapp_documento d
...
//        return $cuadre;
    ...
    public function reporteTipodocPos($nombre, $tipo, $fecha_inicio, $fecha_fin, $caja_id){
        ...
        from gapp_detalle_documento dd
        join gapp_documento d
...
//        $reporte = DB::select("select * from `gapp_documento` where `created_at` >= $fecha_inicio and `created_at` <= $fecha_fin");
    ...
    public function preCuadrePos() {
        ...
        $cuadre = DB::select("
            ...
            $caja_condicion
            and ( gapp_documento.tipodoc = 'BI' or gapp_documento.tipodoc = 'PN'
            or gapp_documento.tipodoc = 'FV'    or gapp_documento.tipodoc = 'FC'
            or gapp_documento.tipodoc = 'RC'    or gapp_documento.tipodoc = 'RT'
            or gapp_documento.tipodoc = 'CI'    or gapp_documento.tipodoc = 'CE' )
            ");

        $fv = DB::select("
            select concat(tp.descripcion,' ',JSON_EXTRACT(pp.obs, '$.tamano')) as descripcion, 
            sum(pp.cant) as cantidad, sum(pp.total) as total, d.numdoc, d.id from gapp_documento d
    ...
    public function getVenderores(Request $request){
        ...
        $data = DB::select("
            ...
            FROM gapp_documento as d
            inner JOIN gapp_pedido as p
            on p.id = d.pedido_id
            inner JOIN gapp_users as u
            on u.id = p.user_id
            WHERE tipodoc = 'FV'
    ...
    public function resumenVentas(){
        if(Input::get('format')=='html'){
            return $this->resumenVentasHtml(Input::all());
        }
        else if(Input::get('format')=='pdf'){
            return $this->resumenVentasHtml(Input::all(), 'pdf');
        }
        else if(Input::get('format')=='pos'){
            return $this->resumenVentasPos(Input::all());
        }
    }
    private function resumenVentasHtml($input, $export_format='html'){

        $printService = app('App\Services\HtmlPrinterService');

        $reportService = app('App\Services\ReportService');
        $posReport = $reportService->resumenDeVentas(
            $input, 0, $printService
        );
        ...
        $html = $printService->print($posReport, $options);
    ...
    private function resumenVentasPos($input){

        $impresora = Config::select('impresora', 'num_impresora')->first()->toArray();
        $printService = app('App\Services\PrinterService');

        $reportService = app('App\Services\ReportService');
        $posReport = $reportService->resumenDeVentas(
            $input, $impresora['num_impresora'], $printService
        );
        $options = [
            'logo' => true
        ];
        $printService->print($impresora, $posReport, $options);
        return back()->with('status', ["success" => "Reporte impreso."]);
    }
}