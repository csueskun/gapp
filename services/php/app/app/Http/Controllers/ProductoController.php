...
//            $producto->valor = $data->valor;
    ...
    public function subirImagen(Request $request){
        ...
        $request->file('_imagen')->move(
            public_path('images/producto'), $imageName
        );
    ...
    public function cambiarImagen(Request $request){
        ...
        if($imageName=="producto.jpg"){
            $imageName = str_slug($producto->descripcion);
            $imageName = substr($imageName, 0, 16);
            $imageName = rtrim($imageName,"-");
            $imageName = $imageName. "-" . $producto->id . '.' . $extension;
        }
        else{
            $imageName = $producto->imagen;
        }
        try {
            ...
            DB::table('producto')->where('id', $producto->id)->update(['imagen' => $imageName]);
            return Redirect::to('producto/editar/'.$producto->id);
        } catch (\Throwable $th) {
            return Redirect::to('producto/editar/'.$producto->id)->with('status', ["warning"=>"No se pudo cambiar la imagen."]);
        }
    }
...
//        }
    ...
    public function adding($id){
        ...
        $ingredientes = DB::select(
            "select i.*, pi.* from gapp_producto_ingrediente pi
