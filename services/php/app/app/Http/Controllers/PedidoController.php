...
//            $valor += $producto->valor;
        ...
        $sql = DB::select("
            select 
                ...
                (
                    ...
                    FROM gapp_producto_pedido_adicional ppa
                    join gapp_adicional as a
                    on a.id = ppa.adicional_id
                    join gapp_ingrediente as i
    ...
    public function calcularValor($id){
        ...
        return DB::table("pedido")
                ->select(DB::raw(
                        "COALESCE((
                                SELECT SUM(PRPE.total) FROM gapp_producto_pedido AS PRPE
                            WHERE PRPE.pedido_id = gapp_pedido.id
                        ),0) as total"
                        ))
                ->where('id', $id)->first();
    }
    ...
    public function actualizarTotales($id){
        DB::select("UPDATE gapp_producto_pedido as x set x.total = x.cant * 
            (x.valor + COALESCE((select sum(total) from gapp_producto_pedido_adicional where producto_pedido_id = x.id),0)) 
            WHERE pedido_id = {$id}");        
    }
    ...
    public function reportePedidosData($fecha_inicio, $fecha_fin, $otherWhere){
        $reporte = DB::select("SELECT
            ...
            FROM gapp_producto_pedido prpe
            JOIN gapp_producto prod on prod.id = prpe.producto_id 
            JOIN gapp_pedido pedi on pedi.id = prpe.pedido_id 
            JOIN gapp_tipo_producto tipr on tipr.id = prod.tipo_producto_id
