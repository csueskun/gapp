...
class ProductoTamanoController extends Controller
{
    ...
    public function todosConProducto($buscar) {
        ...
        return ProductoTamano::select(DB::raw("gapp_producto.id,gapp_producto_tamano.valor, REPLACE(CONCAT_WS(' ',gapp_tipo_producto.descripcion,gapp_producto.descripcion,UPPER(gapp_producto_tamano.tamano)), 'UNICO', '') as detalle"))
                ->join('producto', 'producto.id', '=', 'producto_tamano.producto_id')
                ->join('tipo_producto', 'tipo_producto.id', '=', 'producto.tipo_producto_id')
                ->whereRaw("(gapp_producto.descripcion like '%$buscar%' or gapp_tipo_producto.descripcion like '%$buscar%')")
                ->get();
    }
