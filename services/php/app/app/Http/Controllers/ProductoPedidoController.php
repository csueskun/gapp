...
class ProductoPedidoController extends Controller
{
    ...
    public function preparado($id) {
        $ppedido = ProductoPedido::find($id);
        if($ppedido->preparado == ''){
            $ppedido->preparado = date("Y-m-d H:i:s");;
            $data = array(
                'pedido_id' => $ppedido->pedido_id,
                'mesa' => $ppedido->pedido->mesa_id,
                'id' => $ppedido->id,
                'preparado' => $ppedido->preparado
            );
            app(\App\Services\PushNotifier::class)->sendProductoPedidoPreparado($data);
        }
        else{
