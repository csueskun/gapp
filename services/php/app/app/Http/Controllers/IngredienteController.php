...
class IngredienteController extends Controller
{
    ...
    public function subirImagen(Request $request){
        ...
        if(Input::get('cargar_imagen')==1){
            ...
            $request->file('_imagen')->move(
                public_path('images/ingrediente/'), $imageName
            );
    ...
    public function editar(Request $request){
        ...
        if(Input::get('cargar_imagen')==1){
            ...
            $imageName = $ingrediente->imagen;

            if($imageName=="ingrediente.jpg"){
                $imageName = str_slug($ingrediente->descripcion);
                $imageName = substr($imageName, 0, 16);
                $imageName = rtrim($imageName,"-");
                $imageName = $imageName. "-" . $ingrediente->id . '.' . $extension;
            }
            else{
                $imageName = $ingrediente->imagen;
            }
            try {
                $request->file('_imagen')->move(public_path('images/ingrediente/'), $imageName);
                DB::table('ingrediente')->where('id', $ingrediente->id)->update(['imagen' => $imageName]);
                return back()->with('status', ["success" => "Imagen actualizada correctamente."]);
            } catch (\Throwable $th) {
                return back()->with('status', ["warning" => "Imagen actualizada correctamente."]);
            }
        }
