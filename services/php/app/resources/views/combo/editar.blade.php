...
<script>
    ...
    function borrarComboProducto($id, $tr){
        mostrarFullLoading();
        var tokenData = {_token: _token};
        $.post('/combo-productos/'+$id+"/borrar", tokenData , function(data){
            if(data.status && data.status  == 200){
                $tr.remove();
                $.post('/combo/{{$combo->id}}/recalcular', tokenData, function(datax){
    ...
    function addComboProducto($tr){
        ...
        $.post('/combo-productos', comboProducto, function(data){
            if(data.status && data.status  == 200 && data.producto && data.producto.id){
                ...
                $.post('/combo/{{$combo->id}}/recalcular', {_token: _token}, function(datax){
                    $('input#precio').val(datax.precio);
                    ocultarFullLoading();
                });
            }
