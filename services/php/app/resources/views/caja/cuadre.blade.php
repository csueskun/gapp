...
<section class="borde-inferior form fondo-comun">
    <div class="container">
        ...
        <br>
        <h2 class="titulo">
            <button type='button' class='btn btn-{{$activo?"danger":"success"}}' onclick="siguienteDiaOperativo('{{ csrf_token() }}')">
...
<section class="borde-inferior form fondo-comun">
    <div class="container">
        <h2 class="titulo">Cuadre de Caja Diario
            ...
            <button class="btn btn-purple" id="diario" href="diario" onclick="preEnviarCorreo()">
...
<section class="borde-inferior form fondo-comun">
    <div class="container">
        <h2 class="titulo">Cuadre de Caja Mensual
            ...
            <button class="btn btn-purple" id="mensual" href="mensual" onclick="preEnviarCorreo(2)">
                <span class="glyphicon glyphicon-envelope" aria-hidden="true"></span> Correo Electrónico
            </button>
        </h2>
    </div>
...
<section class="borde-inferior form fondo-comun">
    <div class="container">
        <h2 class="titulo">Cuadre de Caja por Fechas
            ...
            <button class="btn btn-purple" onclick="preEnviarCorreo(3)">
                <span class="glyphicon glyphicon-envelope" aria-hidden="true"></span> Correo Electrónico
            </button>
        </h2>
...
<section class="borde-inferior form fondo-comun">
    <div class="container">
        <h2 class="titulo">Resumen de Ventas por Fechas
            <br/>
            <button class="btn btn-success" href="#" onclick="preResumenVentas()">
                <span class="glyphicon glyphicon-file" aria-hidden="true"></span> PDF
            </button>
            <button class="btn btn-primary" onclick="preResumenVentas('pos', '_self')">
                <span class="fa fa-print" aria-hidden="true"></span> POS
            </button>
            <button disabled class="btn btn-purple" onclick="preResumenVentas('pdf', '_blank')">
                <span class="glyphicon glyphicon-envelope" aria-hidden="true"></span> CORREO ELECTRÓNICO
            </button>
            <button class="btn btn-warning" onclick="preResumenVentas('html', '_blank')">
                <span class="fa fa-code" aria-hidden="true"></span> HTML
            </button>
        </h2>
        <form id="resumen-ventas" action="resumen-ventas" method="POST">
            {{ csrf_field() }}
            <input type="hidden" name="_method" value="POST"/>
            ...
            <div class = "col-md-4 centrado">
                ...
                <div class = "campo tipo fecha form-group has-feedback">
                    ...
                    <div class="datetime-picker start"></div>
                    <input type="hidden" name="fecha_inicio"/>
                </div>
            </div>
            <div class = "col-md-4 centrado">
                ...
                <div class = "campo tipo fecha form-group has-feedback">
                    <span class="preview"></span>
                    <div class="datetime-picker end"></div>
                    <input type="hidden" name="fecha_fin"/>
                </div>
            </div>
...
<script>
    ...
    $(function(){
        ...
        $('div.datetime-picker').datetimepicker({
            language:  'es',
            autoclose: 1,
            todayHighlight: 1,
            minView: 1,
        ...
        $('div.datetime-picker').each(function(i, e) {
            var fecha = new Date();
            if($(e).hasClass('end')) {
                fecha.setHours(fecha.getHours()+1);
            }
            $(e).parent().find("span.preview").html(formatearFecha(fecha, "dd/mm/yyyy hh:00:00"));
            $(e).parent().find("input").val(formatearFecha(fecha, "yyyy-mm-dd hh:00:00"));
        });
        
        $('div.datetime-picker').on('changeMonth changeYear changeHour changeMinute changeDay changeDate', pickDatetime);
        $('div.datetime-picker .datetimepicker-hours th.prev, div.datetime-picker .datetimepicker-hours th.next').on('click', function (e) {
            setTimeout(() => {
                $(e.target).closest('div.datetime-picker').find('.hour.active').click();
            }, 50);
        });
    });
...
<style>
    ...
    label>span, .campo.tipo.fecha>span,
    span.preview{
        display: block;
        font-size: 20px;
        font-weight: bold;
    ...
    <script>
        ...
        function posManual(){
            ...
            //     enviarAServicioImpresion('http://localhost:8000/HtmlPrint?stack='+JSON.stringify(data))
            // });
            impPos({hora: true, fecha_inicio: fechaI, fecha_fin: fechaF, caja_id:$("form#cuadre_ input[name=caja_id]").val(), _token: '{{ csrf_token() }}'});
        }
        ...
        function posResumenVentas(){
            var fechaI = new Date($('div#fecha_inicio').datetimepicker('getFormattedDate'));
            fechaI = formatearFecha(fechaI, "yyyy-mm-dd hh:00:00");
            var fechaF = new Date($('div#fecha_fin').datetimepicker('getFormattedDate'));
            fechaF = formatearFecha(fechaF, "yyyy-mm-dd hh:00:00");

            $.post("/caja/resument-ventas", {fecha_inicio: fechaI, fecha_fin: fechaF}, function (data) {
                console.log(data);
            });
        }
