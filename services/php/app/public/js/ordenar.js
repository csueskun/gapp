...
$(function () {
    ...
    highlightFromParams(new URLSearchParams(window.location.search));
});
    ...
    if(productos_pedido!="{}"&&productos_pedido!=null){
        ...
        for(var i in productos_pedido){
            ...
            if(productos_pedido[i].preparado){
                html+= '        <span title="Preparado" class="btn comandado fa fa-cutlery text-success btn-default preparado"></span>';
            }
...
function itemPreparado(id){
    var pp = $('#ul-pedido>li#'+id);
    if(pp.length==0){
        return false;
    }
    pp.addClass('simple-highlight');
    pp.find('div.items span.preparado').remove();
    pp.find('div.items').prepend('<span title="Preparado" class="btn comandado fa fa-cutlery text-success btn-default preparado"></span>');
    return true;
}

function highlightFromParams(params){
