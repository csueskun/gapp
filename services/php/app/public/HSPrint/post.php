...
foreach($stack as $instruccion){
    if($instruccion->i == 'impresora'){
        ...
        if(getenv('OS') == 'linux'){
            $connector = new CupsPrintConnector($impresora); //linux
        }
        else{
            $connector = new WindowsPrintConnector($impresora); //windows
        }

        // if(getenv('MY_VARIABLE') == 'linux'){
        //     $connector = new CupsPrintConnector($impresora); //linux
        // }
        // else{
        //     $connector = new WindowsPrintConnector($impresora); //windows
    ...
    if($instruccion->i == 'producto_pedido'){
        ...
        $imagen = EscposImage::load('./img/logo-gapp-pos-2025x380-padded.png', false);
