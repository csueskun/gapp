<?php
    ...
    if($limite==-1){
        ...
        $script = "delete from gapp_detalle_documento where id in (".implode(",", $delete).");";
        $result = $conn->query($script);
        foreach ($documentos as $doc) {
            $script = "update gapp_documento set total = gapp_documento.total - ".$doc['restar']." where id = ".$doc['id'].";";
            echo $script;
            echo '<br>';
            $result = $conn->query($script);
        }
